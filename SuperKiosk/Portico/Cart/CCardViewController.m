...
@interface CCardViewController () <UITableViewDataSource, UITableViewDelegate, UIPickerViewDelegate, UIPickerViewDataSource, UITextFieldDelegate>
{
...
- (void)viewDidLoad {
    ...
    */
    _selectedMonth=@(_selectedCard.expMonth);
    
    _selectedYear=@(_selectedCard.expYear);
...
- (IBAction)completeButtonTapped:(id)sender {
    ...
    _selectedCard.name= self.stripeCard.name = self.nameTextField.text;
    _selectedCard.number= self.stripeCard.number = self.cardNumber.text;
    _selectedCard.cvc = self.stripeCard.cvc = self.CVCNumber.text;
    
    self.stripeCard.expMonth = [self.selectedMonth integerValue];
    _selectedCard.expMonth=[self.selectedMonth integerValue];
    
    self.stripeCard.expYear = [self.selectedYear integerValue];
    _selectedCard.expYear=[self.selectedYear integerValue];
    ...
    if (section == 0) {
        if(row==0){
            ...
            if(_selectedCard.name)
                cell.textField.text = _selectedCard.name;
...
//credi card info
        ...
        if(row == 0){
            ...
            if(_selectedCard.number)
                cell.textField.text=_selectedCard.number;
            ...
            cell.textField.placeholder=@"Required";
            if(_selectedCard.cvc)
                cell.textField.text = _selectedCard.cvc;
    ...
    //set default
         ...
         if(row == 0){
            ...
            cell.nameLabel.text =@"Use this card";
             ...
             [cell.enableSwitch setOn:_selectedCard.isDefault];
