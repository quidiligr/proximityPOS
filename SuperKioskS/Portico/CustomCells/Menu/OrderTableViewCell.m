...
+ (void)initialize
{
	if (self == [OrderTableViewCell class])
	{
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString*)reuseIdentifier
{
	if ((self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]))
	{
       ...
       //ROM: DO NOT USE _bubbleView.autoresizingMask = UIViewAutoresizingFlexibleWidth;
...
- (void)setMessage:(MessageTableViewData*)message{
   ...
   // if ([message.userid longValue]==[message.senderid longValue])
    ...
    if([message.user_deviceid isEqualToString:message.sender_deviceid])
	{
    ...
    if ([message.userid longValue]==[message.senderid longValue]){
       ...
       // _label.frame = CGRectMake(_photo.frame.size.width, [message.bubbleSizeheight floatValue], ((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)?TABLE_CELL_WIDTH_IPAD :self.bounds.size.width) - 16, 16);
