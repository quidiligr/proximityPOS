...
@interface MessageWithMediaCell() {
...
@implementation MessageWithMediaCell

+ (void)initialize
{
	if (self == [MessageWithMediaCell class])
	{
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString*)reuseIdentifier
{
	...
	{
		self.selectionStyle = UITableViewCellSelectionStyleBlue;//UITableViewCellSelectionStyleGray; //UITableViewCellSelectionStyleNone;
        ...
        //_imageAttach=[[UIImageView alloc] initWithFrame:CGRectMake(0,0,CELL_IMAGE_WIDTH,CELL_IMAGE_HEIGHT)];
        //[self.contentView addSubview:_imageAttach];
        
        //_btnPlayer=[[UIButton alloc] initWithFrame:CGRectMake(marginLeft,0,CELL_MEDIA_WIDTH,CELL_MEDIA_HEIGHT)];
        if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)
            _imgPlayer = [[UIImageView alloc] initWithFrame:CGRectMake((TABLE_CELL_WIDTH_IPAD/2)-(CELL_MEDIA_WIDTH/2),0,CELL_MEDIA_WIDTH,CELL_MEDIA_HEIGHT)];
        else
            _imgPlayer = [[UIImageView alloc] initWithFrame:CGRectMake((self.bounds.size.width/2)-(CELL_MEDIA_WIDTH/2),0,CELL_MEDIA_WIDTH,CELL_MEDIA_HEIGHT)];
...
//CGFloat marginH=10.0;
    ...
    //[_btnPlayer setImage:message.myMedia forState:UIControlStateNormal];
    ...
    if([message.userid isEqualToNumber:message.senderid])
	{
		...
		//point.x = self.bounds.size.width - ([message.bubbleSizewidth floatValue]+_photo.frame.size.width)- marginLeft;
        ...
        rectPhoto.origin.y=rectMedia.size.height;
        
        
        //_image.frame=rectPhoto;
        
        _label.textAlignment = NSTextAlignmentRight;
       // _image.image=message.myPhoto;
	...
	else
	{
        ...
        point.y=rectMedia.size.height;
         rectPhoto.origin=CGPointMake(marginLeft, 0);
        //rectPhoto.origin=CGPointZero;
        rectPhoto.origin.y=rectMedia.size.height;
        //_image.frame=rectPhoto;
        
		_label.textAlignment = NSTextAlignmentLeft;
       // _image.image=message.otherPhoto;
    ...
    //_label.frame = CGRectMake(8, [message.bubbleSizeheight floatValue]+rectMedia.size.height, self.contentView.bounds.size.width - 16, 16);
    if ([message.userid longValue]==[message.senderid longValue])
        //_label.frame = CGRectMake(0,  [message.bubbleSizeheight floatValue]+rectMedia.size.height, self.contentView.bounds.size.width - _photo.frame.size.width, 16);
        _label.frame = CGRectMake(0,  [message.bubbleSizeheight floatValue]+rectMedia.size.height, ((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)?TABLE_CELL_WIDTH_IPAD :self.bounds.size.width) - _photo.frame.size.width, 16);
    
    else
        _label.frame = CGRectMake(_photo.frame.size.width,  [message.bubbleSizeheight floatValue]+rectMedia.size.height, ((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)?TABLE_CELL_WIDTH_IPAD :self.bounds.size.width) - 16, 16);
}
/*
- (void)setMessage:(Message*)message myPhoto:(UIImage*)myPhoto otherPhoto:(UIImage*)otherPhoto myMedia:(UIButton *)myMedia
{
	CGPoint point = CGPointZero;
    //CGPoint pointPhoto = CGPointZero;
    CGRect rectPhoto=_image.frame;
    CGRect rectImage=_imageAttach.frame;
    CGRect rectMedia=_btnPlayer.frame;
    ...
    if ([message.userid longValue]==[message.senderid longValue])
	{
		...
		point.y=rectImage.size.height;
        //position the photo
        //pointPhoto.x=self.bounds.size.width -_image.frame.size.width;
        //rectPhoto=_image.frame;
        rectPhoto.origin.x=self.bounds.size.width -_image.frame.size.width;
        rectPhoto.origin.y=rectImage.size.height;
	...
	[_label sizeToFit];
    //_label.frame = CGRectMake(8, message.bubbleSize.height, self.contentView.bounds.size.width - 16, 16);
    _label.frame = CGRectMake(8, [message.bubbleSizeheight floatValue]+rectImage.size.height, self.contentView.bounds.size.width - 16, 16);
}
*/
