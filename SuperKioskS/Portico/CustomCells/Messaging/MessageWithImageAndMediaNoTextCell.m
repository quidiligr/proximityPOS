...
#import "MessageWithImageAndMediaNoTextCell.h"
//#import "Message.h"
...
@interface MessageWithImageAndMediaNoTextCell() {
...
@implementation MessageWithImageAndMediaNoTextCell

+ (void)initialize
{
	if (self == [MessageWithImageAndMediaNoTextCell class])
	{
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString*)reuseIdentifier
{
	...
	{
		self.selectionStyle = self.selectionStyle = UITableViewCellSelectionStyleBlue;//UITableViewCellSelectionStyleNone;
...
*/
        ...
        //_imageAttach=[[UIImageView alloc] initWithFrame:CGRectMake(marginLeft,marginTop,CELL_IMAGE_WIDTH,CELL_IMAGE_HEIGHT)];
        if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)
            _imageAttach = [[UIImageView alloc] initWithFrame:CGRectMake((TABLE_CELL_WIDTH_IPAD/2)-(CELL_IMAGE_WIDTH/2),0,CELL_IMAGE_WIDTH,CELL_IMAGE_HEIGHT)];
        else
            _imageAttach = [[UIImageView alloc] initWithFrame:CGRectMake((self.bounds.size.width/2)-(CELL_IMAGE_WIDTH/2),0,CELL_IMAGE_WIDTH,CELL_IMAGE_HEIGHT)];
...
- (void)setMessage:(MessageTableViewData*)message{
    ...
    if([message.userid isEqualToNumber:message.senderid])
	{
		...
		point.y=rectImage.size.height+rectMedia.size.height;
        //position the photo
        ...
        rectPhoto.origin.y=rectImage.size.height+rectMedia.size.height;
   ...
   // _label.frame = CGRectMake(8, [message.bubbleSizeheight floatValue]+rectImage.size.height+rectMedia.size.height, self.contentView.bounds.size.width - 16, 16);
    if ([message.userid longValue]==[message.senderid longValue])
        _label.frame = CGRectMake(0, [message.bubbleSizeheight floatValue]+rectImage.size.height+rectMedia.size.height, ((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)?TABLE_CELL_WIDTH_IPAD :self.bounds.size.width ) - _photo.frame.size.width, 16);
    
    else
        _label.frame = CGRectMake(_photo.frame.size.width, [message.bubbleSizeheight floatValue]+rectImage.size.height+rectMedia.size.height, ((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)?TABLE_CELL_WIDTH_IPAD :self.bounds.size.width ) - 16, 16);
}
/*
- (void)setMessage:(Message*)message myPhoto:(UIImage*)myPhoto otherPhoto:(UIImage*)otherPhoto myImage:(UIImage *)myImage myMedia:(UIButton *)myMedia
{
	CGPoint point = CGPointZero;
    //CGPoint pointPhoto = CGPointZero;
