...
#import "MessageWithImageNoTextCell.h"
...
@interface MessageWithImageNoTextCell() {
...
@implementation MessageWithImageNoTextCell

+ (void)initialize
{
	if (self == [MessageWithImageNoTextCell class])
	{
...
- (void)setMessage:(MessageTableViewData*)message {
    ...
    if([message.userid isEqualToNumber:message.senderid])
	{
		...
		point.y=rectImage.size.height+CELL_VERTICAL_SPACE;
        //position the photo
   ...
   // _label.frame = CGRectMake(8, [message.bubbleSizeheight floatValue]+rectImage.size.height, self.contentView.bounds.size.width - 16, 16);
    if ([message.userid longValue]==[message.senderid longValue])
        _label.frame = CGRectMake(0, [message.bubbleSizeheight floatValue]+rectImage.size.height+CELL_VERTICAL_SPACE, ((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)?TABLE_CELL_WIDTH_IPAD :self.bounds.size.width ) - _photo.frame.size.width, 16);
    
    else
        _label.frame = CGRectMake(_photo.frame.size.width, [message.bubbleSizeheight floatValue]+rectImage.size.height+CELL_VERTICAL_SPACE, ((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)?TABLE_CELL_WIDTH_IPAD :self.bounds.size.width ) - 16, 16);

}

/*
- (void)setMessage:(Message*)message myPhoto:(UIImage*)myPhoto otherPhoto:(UIImage*)otherPhoto myImage:(UIImage *)myImage
{
	CGPoint point = CGPointZero;
    //CGPoint pointPhoto = CGPointZero;
