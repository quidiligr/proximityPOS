...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    UserGroup *g=[_groups objectAtIndex:section];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    NSArray *peers=[_appDelegate.selectedStore.peerInfo.peers filteredArrayUsingPredicate:p];
    */
    UserGroup *g=[_groups objectAtIndex:section];
    if(g.expanded){
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    dispatch_async(imageQueue, ^{
        ...
        if([[peer valueForKey:DeviceIdKey] isEqualToString:_appDelegate.appConfig.userInfo.deviceID])
            ...
            if(_appDelegate.selectedStore.peerInfo==nil)
                return;
            filePath=[[AppDelegate getImagesPathForServer:_appDelegate.selectedStore.peerInfo.deviceID] stringByAppendingPathComponent:peer_photo];
        }
    ...
    if([_appDelegate.appConfig.userInfo.blockedDevices containsObject:[peer valueForKey:DeviceIdKey]]){
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if([[peer valueForKey:DeviceIdKey] isEqualToString:_appDelegate.appConfig.userInfo.deviceID] || [[peer valueForKey:DeviceIdKey] isEqualToString:_appDelegate.selectedStore.peerInfo.deviceID]){
...
-(BOOL)tableView:(UITableView *)tableView canEditRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if([[peer valueForKey:DeviceIdKey] isEqualToString:_appDelegate.selectedStore.peerInfo.deviceID]){
        return NO;
    }
...
-(void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(editingStyle==UITableViewCellEditingStyleDelete){
        ...
        NSDictionary *peer=[peers objectAtIndex:indexPath.row];
        if([_appDelegate.appConfig.userInfo.blockedDevices containsObject:[peer valueForKey:DeviceIdKey]])
            [_appDelegate.appConfig.userInfo.blockedDevices removeObject:[peer valueForKey:DeviceIdKey]];
        else
            [_appDelegate.appConfig.userInfo.blockedDevices addObject:[peer valueForKey:DeviceIdKey]];
...
-(void)reloadData{
    ...
    peerGroupNames=[_appDelegate.selectedStore.peerInfo.peers valueForKeyPath:@"@distinctUnionOfObjects.PeerGroup"];
    
    if([peerGroupNames count]>0){
        BOOL def_expanded=YES;
        
        if(_appDelegate.selectedStore.peerInfo.peers.count>50)
            def_expanded=NO;
        ...
        NSPredicate *p;
        NSString *name;
        for(NSInteger section=0;section<[peerGroupNames count];section++){
            name=[peerGroupNames objectAtIndex:section];
            UserGroup *g=[[UserGroup alloc] initWithName:name expanded:def_expanded];
            
            p=[NSPredicate predicateWithFormat:@"PeerGroup==%@",[peerGroupNames objectAtIndex:section]];
            g.peers=[_appDelegate.selectedStore.peerInfo.peers filteredArrayUsingPredicate:p];
    ...
    */
    ...
    //peerGroupNames=[_appDelegate.selectedStore.peerInfo.peers valueForKey:@"PeerGroup"];
...
-(void)expandGesture:(UITapGestureRecognizer *)sender{
    ...
    UserGroup *g=[_groups objectAtIndex:section];//[_sortedCategories objectAtIndex:section];
