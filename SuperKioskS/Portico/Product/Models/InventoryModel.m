...
+ (InventoryModel *)sharedInstance {
    ...
    dispatch_once(&once, ^{
        ...
        //_sharedInventory.nventories=[NSMutableArray new];
...
//we can also use this load data from the web
- (void)loadInventoryWithData:(NSData *)data{

    NSKeyedUnarchiver* unarchiver = [[NSKeyedUnarchiver alloc] initForReadingWithData:data];
    self.inventory=[unarchiver decodeObjectForKey:@"Inventory"];
...
- (void)loadInventory
{
  ...
  //  [self.categories removeAllObjects];
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:path])
    {
        ...
        [self loadInventoryWithData:data];
    }
    else
    {
       ...
       // [self loadDefaultCategory];
        self.inventory=[InventoryModel createInventory];
    }
    /*
    if([_inventories count]==0){
       // [self createInventory:<#(NSString *)#>]
    }
     */
}
...
- (void)saveInventory{
    ...
    _inventory.inventoryId=newId;
   [archiver encodeObject:self.inventory forKey:@"Inventory"];
...
//    //else
//     */
//    return [inventoryModel.inventories firstObject];
//    //return  nil;
//}
//
...
//added to use only one inventory
+ (InventoryItem *)createInventory//:(NSString *)storeID
{
    ...
    //   return nil;
    /*
    if(self.inventories==nil)
        _inventories=[NSMutableArray new];
    else
    ...
    */
    ...
    //newItem.inventoryId=_appDelegate.appConfig.device_id;
    newItem.name=_appDelegate.appConfig.device_name;
    //set defaults
    newItem.isEcommerce=NO;
    ...
    //[self saveInventory];
    return newItem;//[self.inventories firstObject];
}
...
- (ProductCategory *)addCategory:(NSString*)categoryName imageName:(NSString *)imageFile detail:(NSString *)detail// toInventory:(NSString *)inventoryName
{
    ...
    NSMutableArray *categories=[_inventory valueForKey:categoriesKey];
   ...
   // NSArray *sortedArray=[model.inventories sortedArrayUsingDescriptors:[NSArray arrayWithObject:descriptor]];
    //for(InventoryItem *inv in sortedArray){
        //for (ProductCategory *c in inv.categories) {
            ...
            NSArray* results = [_inventory.products filteredArrayUsingPredicate:predicate];
