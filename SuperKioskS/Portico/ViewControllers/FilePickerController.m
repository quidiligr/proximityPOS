...
- (void)viewDidLoad
{
    ...
    if([self.folder isEqualToString:@"backup"])
        _documentsDirectory=[AppDelegate getBackupPath];
    else if([self.folder isEqualToString:@"images"])
        _documentsDirectory=[AppDelegate getImagesPath];
    else
        _documentsDirectory=[AppDelegate getDocumentsPath];
    //[self copySampleFilesToDocDirIfNeeded];
    _arrFiles = [[NSMutableArray alloc] initWithArray:[self getAllDirFiles]];
...
-(void)copyFilesFromWebToDocDirIfNeeded{
    /*NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    _documentsDirectory = [[NSString alloc] initWithString:[paths objectAtIndex:0]];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    _selectedFile = [_arrFiles objectAtIndex:indexPath.row];
    _selectedRow = indexPath.row;
    [self.delegate filePickerController:self didFinishPickingFileWithInfo:@{@"filename":[_documentsDirectory stringByAppendingPathComponent:_selectedFile]}];
    //[self dismissViewControllerAnimated:YES completion:nil];
    
    //NSString *title=[_selectedFile stringByReplacingOccurrencesOfString:@".pkg" withString:@""];
...
/*
        UIAlertView * alert =[[UIAlertView alloc ] initWithTitle:@"Import Inventory" message:@"Enter title" delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles: nil];
            ...
            NSProgress *progress = [_appDelegate.mcManager.session sendResourceAtURL:resourceURL
                                                               ...
                                                               withCompletionHandler:^(NSError *error) {
                                                                   if (error) {
                                                                       ...
                                                                       UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Here S"
