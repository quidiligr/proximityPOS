...
@implementation OrderListViewController
{
    ...
    //UIButton *chatButton;
    //BBBadgeBarButtonItem *chatBadgeBarButton;
...
- (void)viewDidLoad
{
   ...
   // chatButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 20, 20)];
    
    // Add your action to your button
   // [chatButton addTarget:self action:@selector(showChatAction:) forControlEvents:UIControlEventTouchUpInside];
    
    // Customize your button as you want, with an image if you have a pictogram to display for example
   //     [chatButton setImage:[UIImage imageNamed:@"chat"] forState:UIControlStateNormal];
    
    // Then create and add our custom BBBadgeBarButtonItem
     //   chatBadgeBarButton = [[BBBadgeBarButtonItem alloc] initWithCustomUIButton:chatButton];
...
- (IBAction)actionSubmitOrder:(id)sender {
    [self.delegate orderListViewControllerDidFinish:self];
}
- (IBAction)cancelOrder:(id)sender {
    //[self dismissViewControllerAnimated:YES completion:nil];
    [self.delegate orderListViewControllerWasCancelled:self];
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if([segue.identifier isEqualToString:@"toProductViewController"]){
        ...
        /*NSDictionary *cat=[_selectedInventory.categories objectAtIndex:_selectedIndexPath.section];
        NSArray *sectionProducts = [cat valueForKey:PRODUCTS_KEY];
*/
      
        viewController.selectedProduct= _selectedProduct;//[[Product alloc] initWithDictionary:[sectionProducts objectAtIndex:_selectedIndexPath.row]];
    }
    else{
...
-(IBAction)barcodeAction:(id)sender{
    ...
    if(_appDelegate.isIpad){
        //if(_popOver==nil){
            ...
            //_chatPopover.popoverContentSize=CGSizeMake(self.view.frame.size.width/2, self.view.frame.size.height);
            _popOver.popoverContentSize=CGSizeMake(320.0, self.view.frame.size.height);
            [_popOver presentPopoverFromBarButtonItem:_barcodeButtonItem permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
