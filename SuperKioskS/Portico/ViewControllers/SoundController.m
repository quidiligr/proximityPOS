...
+ (SoundController *)sharedInstance {
    static SoundController*  _sharedSoundController;
    
    static dispatch_once_t once;
    dispatch_once(&once, ^{
        _sharedSoundController = [[SoundController alloc] init];
        
        
    });
    
    return _sharedSoundController;
}
...
- (void)playSystemSound:(SystemSoundID)soundID soundSource:(NSInteger)sourceID {
    ...
    if(sourceID==SOURCE_CHAT && !_appDelegate.appConfig.enableChatSound){
        return;
    }
    else if(sourceID==SOURCE_ORDERS && !_appDelegate.appConfig.enableOrderSound){
        return;
    }
    
    @try{
    if (soundID==SOUND_ORDER_RECEIVED) {
        AudioServicesPlaySystemSound(self.pewPewSound);
    }
    else{
...
- (void)playAlertSound:(SystemSoundID)soundID {
    //AudioServicesPlayAlertSound(self.pewPewSound);
    @try{
    if (soundID==SOUND_ORDER_RECEIVED_ALERT) {
        AudioServicesPlaySystemSound(self.pewPewSound);
    }
    else{
    //sms received alert
